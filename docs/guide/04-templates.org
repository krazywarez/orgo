...
URL of =theme.css=, relative to this page — the [[file:02-configuration.org][built-in theme]] named by
...
A layout that ignores it is a layout with its own CSS, which is the point at which you
have outgrown the setting.

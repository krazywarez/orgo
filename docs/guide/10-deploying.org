...
Everything in =_site= is the site: HTML, the generated =syntax.css=, =theme.css= if the
config names a [[file:02-configuration.org][theme]], and every asset copied from the source. There is no runtime, no server requirement and no build step
downstream.
...
page. Point a =robots.txt= at it if you want one:

#+BEGIN_EXAMPLE
Sitemap: https://example.com/sitemap.xml
#+END_EXAMPLE

=robots.txt= is an ordinary file — put it beside your org files, or in a directory named
by =[build] assets=, and it is copied through.

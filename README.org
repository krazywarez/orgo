...
*Full documentation is in [[file:docs/][=docs/=]]* — a site written in org and built by
...
={% extends %}=. Templates are [[https://docs.rs/minijinja][minijinja]] (Jinja2 syntax) and
receive:
...
=#+SLUG:=. See [[#phase-0-the-corpus-audit-and-the-emacs-oracle][Phase 0]].
...
from =#+SLUG:= when present ([[file:src/util.rs][=util::output_path=]]); slugs are sanitized so an
...
The nav is now built from *top-level pages only* ([[file:src/site.rs][=is_top_level=]]): a nav is a
...
- *Rendered HTML details.* orgo tracks what Emacs exports from the same file, and
  ...
  [[file:CHANGELOG.org][CHANGELOG.org]] — the class names the documentation names (=post-list=,
